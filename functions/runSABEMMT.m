...
%       This code is a is a general-purpose aerodynamic and structural 
%       model for the analysis and optimization of:
%           - Airplane propellers
...
function out = runSABEMMT(v, rpm, Nb, R, c, theta_deg, prop_const, environment)
    ...
    % --- 2.2 Robust Fallback using fzero if neccessary ---
    ...
    if ~isempty(bad_idx)
        for i = bad_idx
             ...
             b = 10 + 10 * abs(mu);
             try
                 % Pass c(i) for Reynolds calculation
    ...
    if abs(P) > 1e-7
        eta_p = v * T / P;
        FoM = Ct*sqrt(0.5*Ct)/Cq; 
    else
        warning('P is close to zero (abs(P) <= tolerance_P).');
        eta_p = NaN; FoM = NaN;
    end
    
    % Reconstruct standard outputs for struct
    Ct_std = Ct * pi^3 / 4; 
    Cq_std = Cq * pi^3 / 8;
    Cp_std = 2 * pi * Cq;
