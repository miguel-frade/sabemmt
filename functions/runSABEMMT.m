...
function out = runSABEMMT(v, rpm, Nb, R, c, theta_deg, prop_const, environment)
    ...
    % --- Enforce row-vector convention for all spanwise quantities ---
    mesh = prop_const.mesh(:).';    % 1×N
    dx   = prop_const.dx(:).';      % 1×N
    c    = c(:).';                  % 1×N
    theta_deg = theta_deg(:).';     % 1×N
    
    % Airfoil geometry points with respect to the centroid (ensure row)
    x_pts = prop_const.structural.x_pts(:).';  % 1×Np
    y_pts = prop_const.structural.y_pts(:).';  % 1×Np

    % Airfoil geometric properties can be vectors if different airfoil
    % geometries are used at each radial station. In that case, enforce
    % those geometric properties are row vectors:
    A_norm   = prop_const.structural.A_norm(:).';
    I_x_norm  = prop_const.structural.I_x_norm(:).';
    I_y_norm  = prop_const.structural.I_y_norm(:).';
    I_xy_norm = prop_const.structural.I_xy_norm(:).';

    % Scale properties to local chord c(r)
    Area_sec = A_norm .* (c.^2);
    I_x_sec  = I_x_norm .* (c.^4);
    I_y_sec  = I_y_norm .* (c.^4);
    I_xy_sec = I_xy_norm .* (c.^4);
    ...
    % --- 4. STRUCTURAL ANALYSIS (Row-native) ---
    ...
    % Element loads (N) per radial bin
    dT  = dTdx  .* dx;      % 1×N
    dFt = dFtdx .* dx;      % 1×N
    
    % --- Integrate Loads (Tip to Root) ---
    S1_dT  = fliplr(cumsum(fliplr(dT)));          % 1×N
    S1_dFt = fliplr(cumsum(fliplr(dFt)));         % 1×N
    Sr_dT  = fliplr(cumsum(fliplr(dT  .* r)));    % 1×N
    Sr_dFt = fliplr(cumsum(fliplr(dFt .* r)));    % 1×N
    
    % Shears (for output)
    V_y_rotor = S1_dT; 
    V_x_rotor = S1_dFt;
    
    % Moments in rotor axes
    M_T       = (Sr_dT  - r .* S1_dT);   % Moment caused by dThrust (1×N)
    M_Ft      = (Sr_dFt - r .* S1_dFt);  % Moment caused by dFt     (1×N)
    ...
    % Transform Moments to Airfoil Axes (using twist theta)
    ...
    cos_ntheta = cos(-theta); 
    sin_ntheta = sin(-theta);
    M_x =  M_x_rotor .* cos_ntheta + M_y_rotor .* sin_ntheta;   % 1×N
    M_y = -M_x_rotor .* sin_ntheta + M_y_rotor .* cos_ntheta;   % 1×N
    
    % --- Centrifugal Stress (row-native) ---
    rho_mat_omega_sq = rho_mat * omega^2;
    integrand = rho_mat_omega_sq .* Area_sec .* r;         % 1×N
    cum_from_root = cumtrapz(r, integrand);                % 1×N
    F_cf_from_i = cum_from_root(end) - cum_from_root;      % 1×N
    safe_A = max(Area_sec, 1e-12);
    sigma_cf = F_cf_from_i ./ safe_A;                      % 1×N
    % sigma_cf(end) = 0; % Optional cleanup
    
    % --- Generalized flexure formula (row-native) ---
    denom = I_x_sec .* I_y_sec - I_xy_sec.^2;              % 1×N
    
    % Curvature-like coefficients Kx, Ky (still 1×N)
    K_x = - (M_y .* I_x_sec - M_x .* I_xy_sec) ./ denom;   % 1×N
    K_y =   (M_x .* I_y_sec - M_y .* I_xy_sec) ./ denom;   % 1×N

    % --- Total stress distribution over perimeter points ---
    % Note: We will output the whole stress distribution at each section
    % Preferred orientation: each COLUMN is one section/airfoil (index i):
    % Define: G(j,i) = Kx(i)*x_pts(j) + Ky(i)*y_pts(j), size: Np×N
    G = (x_pts.' * K_x) + (y_pts.' * K_y);    % (Np×1)*(1×N) + (Np×1)*(1×N) = Np×N

    % Total stress at each perimeter point (Pa), Np×N
    % Note that we have to scale x_pts and y_pts with c:
    % sigma_total_pts(j,i) = sigma_cf(i) + c(i)*G_pts(j,i)
    sigma_total_pts = (G .* c) + sigma_cf; % Np×N
    
    % --- Section maxima and critical point per section ---
    % Maximum total stress per section (Pa)
    % Max over perimeter points (dimension 1), returns 1×N
    [sigma_total_max, jcrit] = max(sigma_total_pts, [], 1);    % 1×N, 1×N
    % At each section (i), the critical point is located at the airfoil pts
    % index (jcrit_col). Note that jcrit a row vector 1xN (one jcrit for 
    % each section (for each i))
    
    % Critical point coordinates per section (normalized)
    % Use jcrit to find the critical point at each section.
    % Note: we transpose jcrit to make it row, and see more clearly the
    % critical point at each section:
    xcrit = x_pts(jcrit);    % 1×N
    ycrit = y_pts(jcrit);    % 1×N
    
    % --- Bending contributions at the SAME critical point (signed) ---
    % bending_total_at_crit = sigma_total_max - sigma_cf
    sigma_bend_max = sigma_total_max - sigma_cf;        % 1×N
    
    % Decompose bending at critical point into the two terms:
    % Note that we have to scale x_pts and y_pts with c:
    % sigma_bend_Iy = c*Kx*xcrit, sigma_bend_Ix = c*Ky*ycrit
    sigma_bend_max_Iy = c .* (K_x .* xcrit);   % 1×N (signed)
    sigma_bend_max_Ix = c .* (K_y .* ycrit);   % 1×N (signed)

    out = struct('R', R, 'rpm', rpm, 'omega', omega, 'theta_deg', theta_deg, ...
                 'theta', theta,'mesh', mesh, 'dx', dx, 'r', r, 'T', T, 'Q', Q, ...
                 'P', P, 'Ct', Ct, 'Cq', Cq, 'Ct_std', Ct_std, 'Cq_std', Cq_std, ...
                 'Cp_std', Cp_std, 'eta_p', eta_p, 'FoM', FoM, ...
                  'c', c, 'phi', phi, 'alpha', alpha, 'Re_local', Re_local, ... 
                 ...
                 'sigma_total_pts', sigma_total_pts, 'idx_crit_per_section', jcrit, ... 
                 'sigma_bend_max', sigma_bend_max, ...
                 'sigma_bend_max_Ix', sigma_bend_max_Ix, ...
                 'sigma_bend_max_Iy', sigma_bend_max_Iy, ...
                 'sigma_total_max', sigma_total_max);
    % Be aware on index convention of sigma_total_pts (each column is a section) (it is intuitive):
    % sigma_total_max = sigma_total_pts(idx_crit_per_section, idx_section)
