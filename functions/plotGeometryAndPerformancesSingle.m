...
function plotGeometryAndPerformancesSingle(v, rpm, Nb, R, c_dist, theta_deg, prop, env)
    ...
    cleanupObj = onCleanup(@() set(0,'DefaultFigureVisible','on'));
    set(0,'DefaultFigureVisible','off');
    drawnow;

    % Open all figures on maximized windows
    % Tell MATLAB to always restore to default, even if an error occurs.
    oldWinState = get(groot,'DefaultFigureWindowState');
    cleanupWinState = onCleanup(@() set(groot,'DefaultFigureWindowState', oldWinState));
    ...
    mainTitle = sprintf('\\textbf{Propeller geometry in 3D \\textemdash\\ %s} ($R=%.0f$ cm)', ...
                        prop.name, R*100);
    ...
    % Figure Super Title
    mainTitle = sprintf('\\textbf{Propeller Geometry Analysis \\textemdash\\ %s} ($R=%.0f$ cm)', ...
                        prop.name, R*100);
    ...
    % GLOBAL TITLE
    % =======================================================================
    if ~exist('titles', 'var'), titles = '3. Propeller Aerodynamics Analysis'; end
    ...
    %% ====================================================================
    %  6. PLOT THE AIRFOIL STRESS DISTRIBUTION
    %  ====================================================================
    %  The first thing to do is to compute the stresses at all points of
    %  each section's upper and lower bounds. 
    %  We have to compute them because SABEMMT does not output these 
    %  stresses (it only outputs the stress in the most critical point of 
    %  each section, even though it internally computes the stresses at 
    %  all points of the section to find the most critical one). The reason 
    %  why SABEMMT does not output these stresses is to reduce overhead.

    % --- Obtain some of the variables needed ---
    % Propeller variables
    dTdx = outputs.dTdx;
    dFtdx = outputs.dFtdx;
    mesh = outputs.mesh;
    ...
    Area_sec = prop.structural.A_norm .* (c.^2);
    I_x_sec  = prop.structural.I_x_norm .* (c.^4);
    I_y_sec  = prop.structural.I_y_norm .* (c.^4);
    I_xy_sec = prop.structural.I_xy_norm .* (c.^4);
    
    
    % --- STRUCTURAL ANALYSIS (General Flexure Formula) ---
    ...
    % Compute Properties
    geo = getAirfoilInertias(x_u, y_u, x_l, y_l);
    ...
    % --- Store full stress distribution for visualization ---
    % NOTE: We store stresses in Pa to keep numerical conditioning
    sigma_total_pts = zeros(length(x_pts), length(r));

    sigma_total_max = zeros(size(r));
    sigma_bend_max_Ix = zeros(size(r));
    sigma_bend_max_Iy = zeros(size(r));
    
    % Pre-calculate the denominator for the Generalized Flexure Formula
    denom = I_x_sec .* I_y_sec - I_xy_sec.^2;
    denom(abs(denom) < 1e-30) = 1e-30; % Avoid singularity. I ~ 1e-22
    ...
    for i = 1:length(r)
        ...
        % Calculate Gradients (Kx, Ky)
        % Formula: sigma_bend = Kx * x + Ky * y
        % Derived from: sigma_bend = - [ (My*Ix - Mx*Ixy)*x + (Mx*Iy - My*Ixy)*y ] / Denom
        K_x = - (My_val * Ix - Mx_val * Ixy) / denom_val;
        K_y = (Mx_val * Iy - My_val * Ixy) / denom_val;
        
        % Note on signs: A positive Moment Mx (Right Hand Rule on X) tractions
        % the upper surface (y>0). So the K_y term must be positive.
        % A positive Moment My (Right Hand Rule on Y) tractions the leading 
        % edge (x<0). So the K_x term must be negative.
        
        % Extract scaled points for this section
        pts_x = x_pts * c(i);
        pts_y = y_pts * c(i);
    
        sigma_total_pts(:,i) = sigma_cf(i) + (K_x .* pts_x + K_y .* pts_y);
        
        % Total Max Tension (CF + Max Bending)
        sigma_total_max(i) = max(sigma_total_pts(:,i));
        
        % Component Breakdown:
        % We save the Maximum Absolute Stress contribution from each axis.
        % This shows how much "Flapwise demand" vs "Edgewise demand" exists.
        sigma_bend_max_Ix(i) = max(abs(K_y .* pts_y)); 
        sigma_bend_max_Iy(i) = max(abs(K_x .* pts_x));
    end
    
    % --- Identify the most critical section (max sigma_total_max) ---
    [~, idx_maxstresssection] = max(sigma_total_max);
    theta_deg = (180/pi) * theta;
    theta_sec_deg = theta_deg(idx_maxstresssection);
    
    % Extract geometry and stresses at critical section (rotate CLOCKWISE by theta)
    x_sec = x_pts * c(idx_maxstresssection);
    y_sec = y_pts * c(idx_maxstresssection);
    sig_sec = sigma_total_pts(:, idx_maxstresssection); % Pa
    
    % Locate maximum stress point on this section
    [sig_max_val, idx_sig_max_pt] = max(sig_sec);
    x_max_pt = x_sec(idx_sig_max_pt);
    y_max_pt = y_sec(idx_sig_max_pt);
    
    % Rotate airfoil CLOCKWISE by theta (i.e., rotation by -theta in math convention)
    th = theta(idx_maxstresssection);
    Rcw = [cos(-th) -sin(-th); sin(-th) cos(-th)];
    XY_rot = Rcw * [x_sec(:)'; y_sec(:)'];
    xR = XY_rot(1,:)';
    yR = XY_rot(2,:)';
    
    XYmax_rot = Rcw * [x_max_pt; y_max_pt];
    xR_max = XYmax_rot(1);
    yR_max = XYmax_rot(2);
    
    % Upper / lower separation for coloring (based on rotated local y)
    idx_upper = (yR >= 0);
    idx_lower = ~idx_upper;
    
    % =====================================================================
    % Plot airfoil stress distribution at 3 sections (side-by-side)
    % =====================================================================
    
    % --- Indices for the 3 sections ---
    [~, idx_crit] = max(sigma_total_max);
    ...
    title_list = { ...
        sprintf('A. Most critical section (r/R=%.3f)', mesh(idx_crit)), ...
        sprintf('B. Mid-span section (r/R=%.2f)', mesh(idx_mid)), ...
        sprintf('C. Characteristic section (r/R=%.2f)',    mesh(idx_075))};
    
    % --- Common color scale across the three subplots (MPa) ---
    sig_all = [];
    for k = 1:numel(idx_list)
        sig_all = [sig_all; sigma_total_pts(:, idx_list(k)) / 1e6]; %#ok<AGROW>
    end
    ...
    for k = 1:3
        ...
        plotAirfoilStressSection(ax_arr(k), idx_list(k), title_list{k}, cax, outputs, prop, sigma_total_pts, M_x, M_y);
    end
    ...
    %  =======================================================================
    fprintf('\n--- Saving plots to results folder ---\n');
    ...
    % Sort them by figure number so they are processed in order
    ...
    for i = 1:length(figHandles)
        ...
        % Get the name defined in the figure
        fName = hFig.Name;
        
        % Fallback if no name is present
        if isempty(fName)
            fName = sprintf('Figure_%d', hFig.Number);
        end
        
        % Sanitize the filename (remove special chars, replace spaces with _)
        % 1. Replace spaces with underscores
        safeName = strrep(fName, ' ', '_');
        % 2. Remove invalid filename characters (keep letters, numbers, underscores)
        safeName = regexprep(safeName, '[^a-zA-Z0-9_]', '');
        % 3. Remove multiple underscores
        safeName = regexprep(safeName, '__+', '_');
        
        % Create full path
        fullFileName = fullfile('results', [safeName, '.jpg']);
        
        % Save as JPG (You can change 'jpeg' to 'png' if preferred)
        saveas(hFig, fullFileName, 'jpeg');
        fprintf('  > Saved: %s\n', fullFileName);
    end
    ...
    % Now that all figures have been saved, show them in numeric order and
    % in maximized windows:
    set(0,'DefaultFigureVisible','on');
    
    figs = findall(0,'Type','figure');
    
    % Get figure numbers and sort descending:
    nums = [figs.Number];
    [numsSorted, idx] = sort(nums, 'descend');
    figsSorted = figs(idx);
    
    % Display in order: figure 1 first, then 2, etc.
    for k = 1:numel(figsSorted)
        hFig = figsSorted(k);
        ...
        figure(hFig);                 % bring to front / make current in correct order
        set(hFig,'Visible','on');     % ensure visible
        drawnow;
    
        % Force maximized (with fallback)
        try
            hFig.WindowState = 'maximized';      % or 'fullscreen'
        catch
...
% =====================================================================
function plotAirfoilStressSection(ax, idx, shortTitle, cax, outputs, prop, sigma_total_pts, M_x, M_y)
    ...
    % Title per subplot
    title(ax, ['\textbf{' shortTitle '}'], 'Interpreter','latex','FontSize',12);
end
